...
public class SyncSampleExtraRepresentationBuilder extends AbstractRepresentationBuilder {
    ...
    @Override
    public int size() {
        return theTrack.getSyncSamples().length;
    }

    @Override
    public Container get(int index) {
        List<Box> moofMdat = new ArrayList<Box>();
        long startSample1 = theTrack.getSyncSamples()[index];
        long startSample2;
        long endSample1;
        long endSample2;
        if (index + 1 < theTrack.getSyncSamples().length) {
            endSample2 = theTrack.getSyncSamples()[index + 1];
        } else {
        ...
        moofMdat.add(createMoof(startSample1, endSample1, theTrack, index * 2 + 1)); // it's one bases
        moofMdat.add(createMdat(startSample1, endSample1));

        if (startSample2 != endSample2) {
            moofMdat.add(createMoof(startSample1, endSample1, theTrack, index * 2 + 2)); // it's one bases
            moofMdat.add(createMdat(startSample1, endSample1));
        }
