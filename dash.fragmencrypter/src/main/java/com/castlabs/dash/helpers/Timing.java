...
public class Timing {
    ...
    public static long getDuration(org.mp4parser.boxes.iso14496.part12.TrackRunBox trun) {
        long[] ptss = new long[trun.getEntries().size()];
        long duration = 0;
        for (int j = 0; j < ptss.length; j++) {
            duration += trun.getEntries().get(j).getSampleDuration();
    ...
    public static long getTimeMappingEditTime(org.mp4parser.Container file) {
        final org.mp4parser.boxes.iso14496.part12.EditListBox editList = org.mp4parser.tools.Path.getPath(file, "moov[0]/trak[0]/edts[0]/elst[0]");
        final org.mp4parser.boxes.iso14496.part12.MediaHeaderBox mdhd = org.mp4parser.tools.Path.getPath(file, "moov[0]/trak[0]/mdia[0]/mdhd[0]");
        final org.mp4parser.boxes.iso14496.part12.MovieHeaderBox mvhd = org.mp4parser.tools.Path.getPath(file, "moov[0]/mvhd[0]");

        if (editList != null) {
            double editStartTime = 0;
            final List<org.mp4parser.boxes.iso14496.part12.EditListBox.Entry> entries = editList.getEntries();
            boolean acceptDwell = true;
            boolean acceptEdit = true;
            for (org.mp4parser.boxes.iso14496.part12.EditListBox.Entry edit : entries) {
                if (edit.getMediaTime() == -1 && !acceptDwell) {
                    throw new RuntimeException("Cannot accept edit list for processing (1)");
                }
                if (edit.getMediaTime() >= 0 && !acceptEdit) {
